...
import com.rest.elk.util.StringUtil;
...
@Api(value="keywordextraction", description="AUTOMATIC KEYWORD EXTRACTION")
public class KeywordExtraction {

	@POST
	@ApiOperation(
			httpMethod = "POST",
			produces = "text/plain", value = "AUTOMATIC KEYWORD EXTRACTION"
			)
	@ApiResponses( value= {
			@ApiResponse(code = 201, message = "Successfully retrieved." , response = Response.class ),
			@ApiResponse(code = 422, message = "Unable to generate response due to faulty(Syntactic or Semantic) XML " ,
			response = Response.class),
			@ApiResponse(code = 404, message = "No Data-Sources configured. No Content found ", response = Response.class)
	})
	public Response getKeyword(@PathParam("doc") String doc) throws IOException {
		 final String text = doc;
	       ...
	       // final Rake rakeInstance = new Rake();
	        ...
	        final Map<String, Double> wordScore = calculateWordScores(candidateList.getPhraseList());
	        final Map<String, Double> keywordCandidates = generateCandidateKeywordScores(candidateList.getPhraseList(), wordScore);

	        System.out.println("keyWordCandidates = "+ keywordCandidates);

	        System.out.println("sortedKeyWordCandidates = " +
	                sortKeyWordCandidates(keywordCandidates));
		return Response.status(200).entity(sortKeyWordCandidates(keywordCandidates).keySet().toArray()[0]).build();
	}
